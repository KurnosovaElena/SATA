...
function StudentProfilePage() {
  ...
  const getTextColor = (widthPercentage, forCount = false) => {
    // Determine if the text should be white or #676B57 based on the width percentage
    const threshold = forCount ? 90 : 15;
    return widthPercentage > threshold ? 'white' : '#676B57';
  };

  const calculateWidth = (count) => {
    const [completed, total] = count.split('/').map(Number);
    return (completed / total) * 100;
  };

  const progressData = [
    { subject: "СВЧВС", count: "10/10" },
    { subject: "СТК", count: "2/8" },
    { subject: "Физика", count: "2/7" },
    { subject: "ТРПО", count: "0/4" },
  ];

  const maxSubjects = 5;
  const emptySlots = maxSubjects - progressData.length;
            ...
            <button className="lab-work-button" onClick={handleLabWork}>
              <div className="lab-work-header">
               ...
               <div className="progress-bar-container">
                {progressData.slice(0, maxSubjects).map((data, index) => {
                  const width = calculateWidth(data.count);
                  return (
                    <div className="progress-bar" key={index}>
                      <div className="progress-filled" style={{ width: `${width}%` }}></div>
                      <span className="progress-text subject" style={{ color: getTextColor(width) }}>{data.subject}</span>
                      <span className="progress-text count" style={{ color: getTextColor(width, true) }}>{data.count}</span>
                    </div>
                  );
                })}
